...
> **Для Codex** — повний промпт для розробки проекту
...
> **Порада**: Перед запуском `Codex` у папці проекту переконайся, що `.claudeignore` існує і містить `node_modules/` та `dist/`. Це зекономить сотні токенів на кожному запиті.

---

## 📋 Задачі для Codex (в порядку виконання)

Використовуй цей список як `/todo` або просто давай Codex по одному блоку:
